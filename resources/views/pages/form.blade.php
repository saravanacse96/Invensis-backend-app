...
<div class="container">
    ...
    <form action="{{ isset($page) ? route('pages.update', $page->id) : route('pages.store') }}" method="POST" enctype="multipart/form-data">
        ...
        <div id="payment-fields" style="{{ (isset($page) && $page->type == 'Payment Page') ? '' : 'display:none;' }}">
            ...
            <div class="form-group mt-2">
                <label for="currency">Currency<span class="mandatory" style="color:red;"> * </span> <span style="color:gray">(Currently accepting usd only)</span></label>
                <input type="text" class="form-control" id="currency" name="currency" value="{{ isset($page) ? $page->currency : 'usd' }}" readonly>
            </div>
        </div>

        <button type="submit" class="btn btn-primary mt-2">{{ isset($page) ? 'Update' : 'Create' }}</button>
    </form>
</div>
@endsection
